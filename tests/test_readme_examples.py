...
class TestREADMEExamples:
    ...
    def test_basic_api_usage_example(self, tmp_path):
        ...
        # Initialize APIKeyPER with a database file and InMemoryBackend for testing
        from apikeyper.config import Config
        from apikeyper.crypto.backends import InMemoryBackend
        
        config = Config(backend=InMemoryBackend())
        api = APIKeyPER(tmp_path / "my_api_keys.db", config)
        ...
        assert github_key_data == "ghp_your_github_token_here"
    ...
    def test_function_decorator_example(self, tmp_path):
        ...
        # Set up test database with InMemoryBackend
        from apikeyper.config import Config
        from apikeyper.crypto.backends import InMemoryBackend
        
        test_db = tmp_path / "func_example.db"
        config = Config(backend=InMemoryBackend())
        api = APIKeyPER(test_db, config)
